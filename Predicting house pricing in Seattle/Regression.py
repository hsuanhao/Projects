...
class LinearRegression:
    ...
    def compute_cost(self, prediction, Y):
        """
        Calculate cost function by using Mean Squared Error (MSE), or L2 loss.
    ...
    def fit(self, X_train, Y_train, learning_rate = 0.0001, num_epochs = 1500, 
            minibatch_size = 32, print_cost = True):
        """
        Implements a linear regression model by minimizing Mean Squared Error (MSE), or L2 loss, cost function.
...
class ExactLinearRegression:
    ...
    def multiple_linear_regression(self, input_feature, output):
        """
        Calculate parameters in W of linear equation y = XW by closed form.
    
        parameters:
        ----------
        input_feature: input feature for x with shape (N,D), where N is number of samples and D is number of features
        output: output for y with shape (N,1)
    
        return:
        ------
        parameters: dictionary {'W':w, 'b':b}
             b : interception of linear equation, W[0]
             W : W[1:D]
        
        Note
        ----
        Although this function can be also applied to one feature case, use simple_linear_regression is much faster.
        
        """
        Y = output
        N, D = input_feature.shape  
        
        X = np.column_stack((np.ones((N,1)) , input_feature))
